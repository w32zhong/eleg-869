...
To follow all the constrains and issues addressed, intuitively, we first take the bind variable with greatest number of occurrence, e.g. ``$a$" with three occurrences in $\left(b \cdot \dfrac{a+b}{a+c} + a\right)$, to match as many as symbols of each bind variable in a document expression in a greedy way. 
