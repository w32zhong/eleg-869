...
\subsection{An matching algorithm}
...
The detailed algorithm is described in figure~\ref{submatchalgo}. The main procedure is \textit{decompose-and-match} where the argument $Q$ and $D$ is the set of leaf-root paths in query tree and document tree respectively, and $C$ is the candidate sets associated with each leaf-root paths in query tree.

\begin{figure}
\begin{algorithmic}[1]
\Procedure{decompose-and-match}{$Q,D, C$} 
\For {$q_k \in Q$}
\If{$|C_k| = 0$}
