...
However, the different ratio of the document expression ``size" to the size the matching query covers essentially result in a different ranking score for eah of the two document expressions.
...
Therefore, in addition to the symbolic similarity degree $s$ given by mark-and-cross algorithm, we combine matching-ratio $r$ to score overall similarity and rank results by the tuple $(s, r)$, that is, first compare $s$ and then $r$ to rank document expressions.
