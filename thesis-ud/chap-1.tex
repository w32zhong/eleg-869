...
Based on an existing idea of using leaf-root path (which will be described at section~\ref{leafrootmethod}) to search mathematical expressions, we have further developed it to index structural information for easy discover of structurally similar document expressions. This paper has proposed new ideas to search math expressions in a limited search set and evaluate both structural and symbolic similarity of two two math expressions.
...
And a newly-assembled corpus from StackExchange to provide a \LaTeX\ (instead of MathML) content data set for public assessment and evaluation. 
\end{itemize}
