...
The second part of our index is a key-value database to map a formula ID to additional information for that formula (e.g. original markup, number of leaf-root paths $|T_d|$ and the URL on which the formula is crawled).
...
\end{center}
\caption{Efficiency Performance}\label{notnecessary}
\end{figure}
...
We have crawled pure \LaTeX\ content from the posts of nearly entire Math Stack Exchange website before March 2015 (over 60MB \textit{bzip2} compressed raw data~\footnote{http://infolab.ece.udel.edu:8912/cowpie/math-stack-exchange-march-2015.tar.bz2} in plain text format with one \LaTeX\ math mode content per line, and one file for each post). 
...
\end{center}
\end{minipage}
\caption{Distribution table}\label{distrtab}
\end{table*}
